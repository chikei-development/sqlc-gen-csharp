...
internal class ModelsGen(DbDriver dbDriver, string namespaceName)
{
    ...
    private MemberDeclarationSyntax[] GenerateDataClasses(
        Dictionary<string, Dictionary<string, Table>> tables
    )
    {
        return (
            ...
            let className = table.Value.Rel.Name.ToModelName(
                table.Value.Rel.Schema,
                dbDriver.DefaultSchema
            )
            select DataClassesGen.Generate(
                className,
                null,
                table.Value.Columns,
                dbDriver.Options,
    ...
    private MemberDeclarationSyntax[] GenerateEnums(
        Dictionary<string, Dictionary<string, Enum>> enums
    )
    {
        return enums
