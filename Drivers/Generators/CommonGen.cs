...
{
    public static string GetMethodParameterList(
        string argInterface,
    ...
    public string ConstructDapperParamsDict(Query query)
    {
        ...
        var initParamsDict =
            $"var {Variable.QueryParams.AsVarName()} = new Dictionary<string, {objectType}>();";
        ...
        var uniqueParams = query.Params.GroupBy(p => p.Column.Name).Select(g => g.First()).ToList();
        ...
        var dapperParamsCommands = uniqueParams.Select(p =>
        {
            ...
            if (
                dbDriver is EnumDbDriver enumDbDriver
                && enumDbDriver.Enums.ContainsKey(p.Column.Type.Name)
            )
                param += "?.ToEnumString()";
            ...
            var paramToWrite = writerFn is null
                ...
                : writerFn(
                    ...
                    p.Column.Type.Name,
                    notNull,
                    dbDriver.Options.UseDapper,
                    dbDriver.Options.DotnetFramework.IsDotnetLegacy()
                );
    ...
    {
        if (!query.Params.Any(p => p.Column.IsSqlcSlice))
            return string.Empty;
        var initVariable = $"var {Variable.TransformedSql.AsVarName()} = {queryTextConstant};";

        var sqlcSliceCommands = query
            .Params.Where(p => p.Column.IsSqlcSlice)
            .Select(c =>
            {
    ...
    public string InstantiateDataclass(Column[] columns, string returnInterface, Query query)
    {
        ...
        foreach (var column in columns)
        {
            ...
            columnsInit.Add(
                $"{tableFieldName} = {InstantiateDataclassInternal(tableFieldType, tableColumnsInit)}"
            );
        ...
        {
            var schemaName =
                tableColumn.EmbedTable.Schema == dbDriver.DefaultSchema
                    ? string.Empty
                    : tableColumn.EmbedTable.Schema;
