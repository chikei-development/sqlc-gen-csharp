...
public class ExecLastIdDeclareGen(DbDriver dbDriver)
{
    ...
    private string GetDapperNoTxBody(string sqlVar, Query query)
    {
        ...
        return $$"""
               using ({{establishConnection}})
                   return await {{Variable.Connection.AsVarName()}}.QuerySingleAsync<{{dbDriver.GetIdColumnType(
                query
            )}}>({{sqlVar}}{{dapperArgs}});
            """;
    }
    ...
    private string GetDapperWithTxBody(string sqlVar, Query query)
    {
        ...
        return $$"""
               ...
               return await this.{{transactionProperty}}.Connection.QuerySingleAsync<{{dbDriver.GetIdColumnType(
                query
            )}}>({{sqlVar}}{{dapperArgs}}, transaction: this.{{transactionProperty}});
            """;
    }
    ...
    private string GetDriverNoTxBody(string sqlVar, Query query)
    {
        ...
        return dbDriver.GenerateNoTransactionMethodBody(sqlVar, query, returnLastId);
    }
